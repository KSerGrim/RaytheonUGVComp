...
double kpfr = 20.0; double kifr = 25.0; double kdfr = 7.5;
double kpfl = 20.0; double kifl = 30.0; double kdfl = 6.75;
double kpbr = 20.0; double kibr = 30.0; double kdbr = 6.75;
double kpbl = 20.0; double kibl = 25.0; double kdbl = 7.75;
...
void loop() {
  ...
  // Serial.println(Setpoint_LeftTracks);
  ...
  if (ppsFlag) {
    ...
    readGPS();
    if (GPS.fix) {
      // display_GPS_Data_On_LCD(MPH_FR, MPH_BL);
    } else {
    //   lcd.clear();
    //   lcd.setCursor(0, 1);
    //   lcd.print("No Signal");
...
float Mcali[3]; // Calibrated magnetometer readings

float Mmeas[3]; // To store raw magnetometer readings (x, y, z)

void readRawMagnetometerData() {
  int x, y, z;
  // Read compass values
  compass.read();
  // Return XYZ readings
  ...
  Mmeas[0] = x, Mmeas[1] = y, Mmeas[2] = z;
  // Serial.print("Azimuth Comp: ");
...
void applyCalibration() {
  ...
  for (int i = 0; i < 3; i++) {
    Mcali[i] = 0; // Initialize to ensure clean calculation
    for (int j = 0; j < 3; j++) {
      Mcali[i] += A[i][j] * Mtemp[j];
    }
  }
}

float calculateAzimuth() {
  applyCalibration(); // Make sure calibrated readings are ready
  
  // Assuming Mcali[0] is X, Mcali[1] is Y
  float azimuth = atan2(Mcali[1], Mcali[0]) * (180.0 / PI); // Convert from radians to degrees
...
void display_GPS_Data_On_LCD(double mphFR, double mphBL) {
...
void init_Compass_with_PID() {
  ...
  //headingPID.SetOutputLimits(0, 3); //3mph control limit
  Setpoint_Comp = 0;
  compass.init();
