...
//Or use equation Pulses per Revolution = (360 / Pulses per Unit Angle)
//Amazon Datasheet says 540 Pulses per Revolution

float distancePerRevolution = 5.0; // Change this based on your setup
float encoderPulses = 6.0;

float distance = (pulsesPerRevolution / encoderPulses)  / distancePerRevolution;
...
double distanceYards = 50.0;

// Turning times for each lane
const unsigned long turningTimes[] = {30000, 100000, 170000, 240000}; // in milliseconds
...
double calculateSetpoint() {
  ...
  double desiredSpeed = 12 / 12.0; // 12 seconds per yard to yards per second
  return desiredSpeed;
}
