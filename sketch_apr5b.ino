...
double default_LeftTracks = .17, default_BL = .17;
double default_RightTracks = .17;
//kpki = 100 kd = 5.25
double kp = 75.0;
double ki = 75.0;
double kd = 5.75;
...
double kdbr = 5.75;
// Kp Ki Kd
PID pid_FR(&MPH_FR, &Input_FR, &Setpoint_RightTracks, kp, ki, kd, DIRECT);  //IS FRONT LEFT
PID pid_FL(&MPH_FL, &Input_FL, &Setpoint_LeftTracks, kp, ki, kd, DIRECT);   //IS FRONT RIGHT
//  Kd Kp Ki
PID pid_BR(&MPH_BR, &Input_BR, &Setpoint_RightTracks, kpbr, kibr, kdbr, DIRECT);

// double aggKp = 18.5, aggKi = 45.0, aggKd = 9.25;
// double aggKp = 17.5, aggKi = 55.0, aggKd = 7.50;
double consKp = 4.5, consKi = 0.25, consKd = 10.0;
double aggKp = 39.0, aggKi = 2.5, aggKd = 60.0;

// PID pid_BL(&MPH_BL, &Input_BL, &Setpoint_LeftTracks, kp, ki, kd, DIRECT);
PID pid_BL(&MPH_BL, &Input_BL, &Setpoint_LeftTracks, aggKp, aggKi, aggKd, DIRECT);
...
void loop() {
  ...
  if (millis() - currentTime >= 100) {
    ...
    // Serial.print(FrontBPWM);
    // Serial.print(" ");
    // Serial.print(FrontAPWM);
    // Serial.print(" ");
    // Serial.print(BackBPWM);
    // Serial.print(" ");
    // Serial.print(BackAPWM);
    // Serial.println();
...
void check_Water_Sensor(double cap_pf) {
  if (cap_pf >= 400.0) {  // Trigger visual and audio cues
